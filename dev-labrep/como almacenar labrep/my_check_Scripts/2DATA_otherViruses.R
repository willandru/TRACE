...
other_viruses <- 'OTROSVIRUS_SE 23.xlsx'
...
other_viruses_data <- import_data_viral_circulation(report_data = other_viruses, skip = 0, sheet = 1)
...
get_cases_other_viruses <- function(report_data,epiweek = NULL,age_groups = FALSE,vrs_influenza = NULL) {
  config_path <- system.file("extdata", "config.yml", package = "labrep")
...
generate_age_groups_viruses <- function(report_data,event_name = "adenovirus", wt_percentage = FALSE,total_cases = 0, event_label) {
...
other_viruses_age_groups <- get_cases_other_viruses(report_data = other_viruses_cleaned_data, age_groups = TRUE)
